...
class UserProgress {
    ...
    loadFromStorage() {
        try {
            ...
            if (savedData) {
                ...
                debug.log("User data loaded from localStorage");
            } else {
                debug.log("No user data found in localStorage");
            }
            this.initialized = true;
        } catch (error) {
            debug.error("Error loading user data: " + error.message);
            this.users = {};
            this.initialized = true;
        }
    }
    ...
    saveToStorage() {
        try {
            ...
            debug.log("User data saved to localStorage");
            return true;
        } catch (error) {
            debug.error("Error saving user data: " + error.message);
            return false;
        }
    }
    ...
    getUser(username, pin, createIfNotExists = false) {
        // For guest user
        if (username === 'Guest') {
            debug.log("Creating guest user session");
            return {
                username: 'Guest',
                pin: '0000',
                stats: {
        ...
        if (this.users[username]) {
            ...
            if (this.users[username].pin === pin) {
                debug.log(`User found: ${username}`);
                return this.users[username];
            } else {
                debug.warn(`Invalid PIN for user: ${username}`);
    ...
    register(username, pin) {
        try {
            ...
            if (this.users[username]) {
                debug.warn(`Registration failed: User ${username} already exists`);
                return false;
            }
    ...
    updateStats(questionId, isCorrect, category) {
        if (!this.currentUser) {
            debug.warn("Can't update stats: No user logged in");
            return;
        }
        
        // Skip saving for guest user
        if (this.currentUser.username === 'Guest') {
            debug.log("Stats not saved for Guest user");
            return;
        }
        ...
        try {
            ...
            // Save changes
            this.saveToStorage();
            debug.log(`Stats updated for user ${this.currentUser.username}: ${isCorrect ? 'Correct' : 'Incorrect'} answer in ${category}`);
        } catch (error) {
            debug.error(`Error updating stats: ${error.message}`);
        }
    }
    ...
    getStats() {
        if (!this.currentUser) {
            debug.warn("Can't get stats: No user logged in");
            return null;
        }
...
const debug = {
    ...
    log: function(message) {
        ...
        this._addToDebugConsole('info', message);
    },
    warn: function(message) {
        if (!this.enabled) return;
        console.warn(`[WARN] ${message}`);
        this._addToDebugConsole('warn', message);
    },
    error: function(message) {
        if (!this.enabled) return;
        console.error(`[ERROR] ${message}`);
        this._addToDebugConsole('error', message);
    },
    _addToDebugConsole: function(type, message) {
